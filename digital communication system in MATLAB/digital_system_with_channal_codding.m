...
M=2;  %constant% M = Number of layers example: m=2 -> [-1 1] m=3 -> [-2 0 2] m=4 -> [-3 -1 1 3] 
n=5; % n = Number of bits generated
...
%alfa*exp(-1*(1/beta)*tx)
alfa = 0.1; % domain of channel
beta = 0.03; % frequency of channel is beta = 10rad.hz
...
%%%%%%%%%%%%%%% HT filter
    ...
    if 1==1
        ...
        subplot(5,1,2)
        plot(tx,g)
        title('signal after transmit filter')
        ylabel('y')
        xlabel('t')
...
%%%%%%%%%%%%%%% HR filter
    ...
    if 1==1
        ...
        subplot(5,1,5)
        plot(tx,g)
        title('signal after reciver filter')
        ylabel('y')
        xlabel('t')
        y=g;
    end
